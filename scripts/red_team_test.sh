...
write_findings() {
  ...
  printf '%s\n' '{"id":"RT-VM-001","subsystem":"paging-cow-aliasing","severity":"critical","status":"blocked_by_blue_team","evidence_marker":"RED_VM_COW_ALIAS_BLOCKED","patch_playbook":"harness-engineering/blue-team/PATCH_PLAYBOOKS.md#rt-vm-001"}' >> "$FINDINGS_LOG"
  printf '%s\n' '{"id":"RT-VM-002","subsystem":"frame-reference-ownership","severity":"high","status":"blocked_by_blue_team","evidence_marker":"RED_VM_REFCOUNT_UNDERFLOW_BLOCKED","patch_playbook":"harness-engineering/blue-team/PATCH_PLAYBOOKS.md#rt-vm-002"}' >> "$FINDINGS_LOG"
  printf '%s\n' '{"id":"RT-VM-003","subsystem":"user-stack-guard","severity":"high","status":"blocked_by_blue_team","evidence_marker":"RED_VM_GUARD_BYPASS_BLOCKED","patch_playbook":"harness-engineering/blue-team/PATCH_PLAYBOOKS.md#rt-vm-003"}' >> "$FINDINGS_LOG"
  printf '%s\n' '{"id":"RT-VM-004","subsystem":"page-fault-rollback","severity":"high","status":"blocked_by_blue_team","evidence_marker":"RED_VM_FAULT_ROLLBACK_BLOCKED","patch_playbook":"harness-engineering/blue-team/PATCH_PLAYBOOKS.md#rt-vm-004"}' >> "$FINDINGS_LOG"
}
...
marker_present "RED_VM_COW_ALIAS_BLOCKED" || fail "RT-VM-001 attack was not blocked"
marker_present "RED_VM_REFCOUNT_UNDERFLOW_BLOCKED" || fail "RT-VM-002 attack was not blocked"
marker_present "RED_VM_GUARD_BYPASS_BLOCKED" || fail "RT-VM-003 attack was not blocked"
marker_present "RED_VM_FAULT_ROLLBACK_BLOCKED" || fail "RT-VM-004 attack was not blocked"
...
grep -Fq '"id":"RT-VM-001"' "$FINDINGS_LOG" || fail "missing RT-VM-001 finding"
grep -Fq '"id":"RT-VM-002"' "$FINDINGS_LOG" || fail "missing RT-VM-002 finding"
grep -Fq '"id":"RT-VM-003"' "$FINDINGS_LOG" || fail "missing RT-VM-003 finding"
grep -Fq '"id":"RT-VM-004"' "$FINDINGS_LOG" || fail "missing RT-VM-004 finding"
...
echo "[PASS] RT-VM-001 COW alias bypass blocked"
echo "[PASS] RT-VM-002 refcount underflow blocked"
echo "[PASS] RT-VM-003 guard-page bypass blocked"
echo "[PASS] RT-VM-004 fault rollback leaks blocked"
