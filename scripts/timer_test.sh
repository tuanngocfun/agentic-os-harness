...
SERIAL_LOG="$BUILD_DIR/serial.timer.log"
SERIAL_CLEAN="$BUILD_DIR/serial.timer.clean.log"
QEMU_LOG="$BUILD_DIR/qemu.timer.log"
EVIDENCE_LOG="$BUILD_DIR/evidence.jsonl"
TIMEOUT_SECONDS="${TIMEOUT_SECONDS:-10}"
TASK_NAME="${TASK_NAME:-timer-test}"
...
write_evidence() {
  ...
  marker_present "TIMER_TICKS_OK" && timer_ticks_ok=true

  task_json="$(json_escape "$TASK_NAME")"
  printf '{"run_id":"%s","task":"%s","started_at":"%s","ended_at":"%s","qemu_status":%s,"artifacts":[{"path":"build/boot.bin","bytes":"%s","sha256":"%s"},{"path":"build/kernel.bin","bytes":"%s","sha256":"%s"}],"serial_log_sha256":"%s","markers":{"BOOT_OK":%s,"KERNEL_INIT_OK":%s,"SHELL_READY":%s},"subsystem":{"TIMER_TICKS_OK":%s},"verdict":"%s"}\n' \
    "$run_id" "$task_json" "$started_at" "$ended_at" "$qemu_status" \
    "$boot_bytes" "$boot_sha" "$kernel_bytes" "$kernel_sha" "$serial_sha" \
    "$boot_ok" "$kernel_ok" "$shell_ready" "$timer_ticks_ok" "$verdict" >> "$EVIDENCE_LOG"
}
...
if marker_present "TIMER_TICKS_FAIL"; then
  ended_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  write_evidence "$run_id" "$started_at" "$ended_at" "$qemu_status" "fail"
  fail "timer selftest reported TIMER_TICKS_FAIL"
fi

if marker_present "TIMER_TICKS_OK"; then
  echo "[PASS] timer ticks increment verified"
else
  ended_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  write_evidence "$run_id" "$started_at" "$ended_at" "$qemu_status" "fail"
  fail "timer ticks not verified - missing TIMER_TICKS_OK"
fi
