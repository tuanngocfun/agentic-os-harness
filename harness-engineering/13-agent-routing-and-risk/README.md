...
Not proven:
- Syscall ABI correctness.
- Exception and panic paths.
- Timer-driven scheduler/context switching.
- Ring-3 user-mode transition.
- Real paging semantics and isolation.
...
| Credible protected OS core | 30% | GDT/IDT exist, but syscall, scheduler, paging, user mode, and fault handling are not runtime-proven. |
...
| `paging_semantics` | Page tables, allocator for page tables, permissions, invalidation | Map/unmap and page-fault tests; no hardcoded reusable page-table address | Memory protection until page-fault and user/supervisor permission tests pass |
...
3. `scheduler-truth`  
   Either remove the implication that scheduler/process are working, or wire a timer-driven scheduling path and prove two contexts run. The current `scheduler_add()` call is not enough.

4. `paging-semantics`  
   Replace the hardcoded page table allocation pattern with a controlled allocator strategy. Add invalidation and permission tests before any isolation claim.
