...
guard-paths:
	...
	@test "$(KERNEL_MAX_CHS_SECTORS)" = "120" || { echo "ERROR: phase-1 BIOS-geometry CHS profile requires KERNEL_MAX_CHS_SECTORS=120"; exit 1; }
...
7. Generate build/boot_config.inc from build/kernel.bin size
   ...
   -> For the BIOS-geometry CHS loader, fail if this exceeds 120 sectors.
...
- The current phase-1 CHS bootloader uses BIOS-reported disk geometry and must keep `KERNEL_SECTORS <= 120`; larger kernels require LBA or a 2-stage loader.
...
| Kernel exceeds 120 sectors | Phase-1 BIOS-geometry CHS loader profile too small | Switch to LBA or a 2-stage loader |
