...
require_file "red-team/README.md"
require_file "red-team/ATTACK_CATALOG.md"
require_file "blue-team/README.md"
require_file "blue-team/CURRENT_FINDINGS.md"
require_file "blue-team/PATCH_PLAYBOOKS.md"
require_file "12-git-change-management/README.md"
require_file "13-agent-routing-and-risk/README.md"
require_file "14-meta-loop/README.md"
...
require_file "../scripts/red_team_test.sh"
require_file "../scripts/marker_surface_test.sh"
require_file "../scripts/exec_args_test.sh"
require_file "../scripts/vm_test.sh"
require_file "../scripts/qemu_runtime.sh"
require_file "../scripts/qemu_runtime_preflight_test.sh"
require_file "../scripts/static_analysis_test.sh"
require_file "../scripts/tooling_red_team_test.sh"
require_file "../.vscode/c_cpp_properties.json"
require_file "../agentic-os.code-workspace"
...
require_grep 'red-team/README[.]md' "llms.txt"
require_grep 'blue-team/README[.]md' "llms.txt"
require_grep '12-git-change-management/README[.]md' "llms.txt"
require_grep '13-agent-routing-and-risk/README[.]md' "llms.txt"
require_grep '14-meta-loop/README[.]md' "llms.txt"
...
require_grep 'status_last_verified:[[:space:]]*"2026-06-21"' "harness_profile.yaml"
...
require_grep 'project_phase:[[:space:]]*"boot_to_shell_proven_stage2_preemptive_memory_ramdisk_vfs_simplefs_file_syscalls_elf_loader_process_lifecycle_per_process_fds_marker_trust_exec_args_red_blue_qemu_evidence_cow_demand_guard_vm"' "harness_profile.yaml"
...
require_grep 'claim:[[:space:]]*"known_red_team_attacks_blocked_security_not_complete"' "harness_profile.yaml"
require_grep 'mode:[[:space:]]*"red_blue_regression_blocking"' "harness_profile.yaml"
require_grep 'adversarial_gates:' "harness_profile.yaml"
require_grep 'make test-red-team' "harness_profile.yaml"
require_grep 'make test-blue-team' "harness_profile.yaml"
...
if grep -q 'ENABLE_VM_SELFTEST' "$REPO_ROOT/kernel/kernel.c" && grep -q 'test-vm' "$REPO_ROOT/Makefile" && grep -q 'VM_OK' "$REPO_ROOT/scripts/vm_test.sh"; then
  require_grep 'virtual_memory:[[:space:]]*"claimable_with_cow_demand_guard_and_rollback_runtime_test"' "harness_profile.yaml"
  require_grep 'test-deep:.*test-vm' "$REPO_ROOT/Makefile"
  require_grep 'PAGE_COW' "$REPO_ROOT/include/paging.h"
  require_grep 'paging_resolve_cow_fault' "$REPO_ROOT/kernel/paging.c"
  require_grep 'frame_retain' "$REPO_ROOT/kernel/frame.c"
  require_grep 'frame_release' "$REPO_ROOT/kernel/frame.c"
  require_grep 'vm_handle_page_fault' "$REPO_ROOT/kernel/vm.c"
  require_grep 'VM_COW_SHARED_OK' "$REPO_ROOT/scripts/vm_test.sh"
  require_grep 'VM_COW_SPLIT_OK' "$REPO_ROOT/scripts/vm_test.sh"
  require_grep 'VM_DEMAND_ZERO_OK' "$REPO_ROOT/scripts/vm_test.sh"
  require_grep 'VM_GUARD_TERMINATION_OK' "$REPO_ROOT/scripts/vm_test.sh"
  require_grep 'VM_FRAME_ACCOUNTING_OK' "$REPO_ROOT/scripts/vm_test.sh"
  require_grep 'VM_CLONE_ROLLBACK_OK' "$REPO_ROOT/scripts/vm_test.sh"
  require_grep 'VM_COW_OOM_ROLLBACK_OK' "$REPO_ROOT/scripts/vm_test.sh"
  require_grep 'VM_DEMAND_OOM_ROLLBACK_OK' "$REPO_ROOT/scripts/vm_test.sh"
else
  require_grep 'virtual_memory:[[:space:]]*"not_claimable_vm_runtime_unproven"' "harness_profile.yaml"
fi
...
if grep -q 'ENABLE_VFS_SELFTEST' "$REPO_ROOT/kernel/kernel.c" && grep -q 'test-vfs' "$REPO_ROOT/Makefile" && grep -q 'VFS_OK' "$REPO_ROOT/scripts/vfs_test.sh"; then
  require_grep 'filesystem:[[:space:]]*"claimable_with_vfs_simplefs_runtime_test"' "harness_profile.yaml"
...
if grep -q 'ENABLE_USERMODE_SELFTEST' "$REPO_ROOT/kernel/kernel.c" && grep -q 'test-usermode' "$REPO_ROOT/Makefile" && grep -q 'USERMODE_RING3_OK' "$REPO_ROOT/scripts/usermode_test.sh" && rg -q 'enter_user_mode|switch_to_usermode' "$REPO_ROOT/kernel" "$REPO_ROOT/include" 2>/dev/null; then
  ...
  if grep -q 'ENABLE_PROCESS_LIFECYCLE_SELFTEST' "$REPO_ROOT/kernel/kernel.c" && grep -q 'test-process-lifecycle' "$REPO_ROOT/Makefile" && grep -q 'LIFECYCLE_OK' "$REPO_ROOT/scripts/process_lifecycle_test.sh"; then
    if grep -q 'ENABLE_PROCESS_FD_SELFTEST' "$REPO_ROOT/kernel/kernel.c" && grep -q 'test-process-fd' "$REPO_ROOT/Makefile" && grep -q 'PROCESS_FD_OK' "$REPO_ROOT/scripts/process_fd_test.sh"; then
      require_grep 'process:[[:space:]]*"claimable_with_ring3_fork_wait_exit_reap_transactional_exec_argv_envp_and_per_process_fd_tests"' "harness_profile.yaml"
...
if grep -q 'ENABLE_REDTEAM_SELFTEST' "$REPO_ROOT/kernel/kernel.c" && grep -q 'test-red-team' "$REPO_ROOT/Makefile" && grep -q 'test-blue-team' "$REPO_ROOT/Makefile" && grep -q 'RED_DEFENSES_OK' "$REPO_ROOT/scripts/red_team_test.sh"; then
  require_grep 'security_posture:' "harness_profile.yaml"
  require_grep 'known_red_team_attacks_blocked_security_not_complete' "harness_profile.yaml"
  require_grep 'red_blue_regression_blocking' "harness_profile.yaml"
  require_grep 'blocked_findings:' "harness_profile.yaml"
  require_grep 'make test-red-team' "harness_profile.yaml"
  require_grep 'make test-blue-team' "harness_profile.yaml"
  require_grep 'RT-HARNESS-001' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-HARNESS-003' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-EXEC-001' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-FS-001' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-FS-002' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-EXEC-002' "red-team/ATTACK_CATALOG.md"
  ...
  require_grep 'RT-VM-001' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-VM-002' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-VM-003' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-VM-004' "red-team/ATTACK_CATALOG.md"
  require_grep 'RT-HARNESS-001' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-HARNESS-003' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-EXEC-001' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-FS-001' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-FS-002' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-EXEC-002' "blue-team/CURRENT_FINDINGS.md"
  ...
  require_grep 'RT-VM-001' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-VM-002' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-VM-003' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-VM-004' "blue-team/CURRENT_FINDINGS.md"
  require_grep 'RT-HARNESS-001' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-HARNESS-003' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-EXEC-001' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-FS-001' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-FS-002' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-EXEC-002' "blue-team/PATCH_PLAYBOOKS.md"
  ...
  require_grep 'RT-VM-001' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-VM-002' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-VM-003' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-VM-004' "blue-team/PATCH_PLAYBOOKS.md"
  require_grep 'RT-HARNESS-001' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RT-HARNESS-003' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RT-EXEC-001' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RT-FS-001' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RT-FS-002' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RT-EXEC-002' "$REPO_ROOT/scripts/red_team_test.sh"
  ...
  require_grep 'RT-VM-001' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RT-VM-002' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RT-VM-003' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RT-VM-004' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_MARKER_FORGERY_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_MARKER_REPLAY_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_SYSCALL_PRIVILEGE_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_EXEC_RESIDUAL_MAPPING_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_SCHED_YIELD_MIXING_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_SIMPLEFS_DOS_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_VFS_NAMESPACE_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_EXEC_FD_LEAK_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  ...
  require_grep 'RED_VM_COW_ALIAS_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_VM_REFCOUNT_UNDERFLOW_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_VM_GUARD_BYPASS_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  require_grep 'RED_VM_FAULT_ROLLBACK_BLOCKED' "$REPO_ROOT/scripts/red_team_test.sh"
  ...
  require_grep 'RED_EXEC_RESIDUAL_MAPPING_BLOCKED' "red-team/README.md"
  require_grep 'RED_SCHED_YIELD_MIXING_BLOCKED' "red-team/README.md"
  require_grep 'RED_SIMPLEFS_DOS_BLOCKED' "red-team/README.md"
  require_grep 'RED_VFS_NAMESPACE_BLOCKED' "red-team/README.md"
  require_grep 'RED_EXEC_FD_LEAK_BLOCKED' "red-team/README.md"
  ...
  require_grep 'RED_VM_COW_ALIAS_BLOCKED' "red-team/README.md"
  require_grep 'RED_VM_REFCOUNT_UNDERFLOW_BLOCKED' "red-team/README.md"
  require_grep 'RED_VM_GUARD_BYPASS_BLOCKED' "red-team/README.md"
  require_grep 'RED_VM_FAULT_ROLLBACK_BLOCKED' "red-team/README.md"
  ...
  require_grep 'RT-HARNESS-003' "harness_profile.yaml"
  require_grep 'RT-FS-002' "harness_profile.yaml"
  require_grep 'RT-EXEC-002' "harness_profile.yaml"
  ...
  require_grep 'RT-VM-001' "harness_profile.yaml"
  require_grep 'RT-VM-002' "harness_profile.yaml"
  require_grep 'RT-VM-003' "harness_profile.yaml"
  require_grep 'RT-VM-004' "harness_profile.yaml"
  ...
  require_grep 'path\[0\][[:space:]]*!=' "$REPO_ROOT/kernel/simplefs.c"
  require_grep 'ranges_overlap' "$REPO_ROOT/kernel/elf.c"
else
  fail "red/blue adversarial regression gate is incomplete"
fi

require_grep "qemu_runtime:" "harness_profile.yaml"
require_grep "mandatory:[[:space:]]*true" "harness_profile.yaml"
require_grep "override:[[:space:]]*\"QEMU_BIOS_DIR\"" "harness_profile.yaml"
require_grep "auto_discovery:[[:space:]]*false" "harness_profile.yaml"
require_grep "fallback:[[:space:]]*false" "harness_profile.yaml"
require_grep "skip_path:[[:space:]]*false" "harness_profile.yaml"
require_grep "QEMU emulator version 4[.]2[.]1" "$REPO_ROOT/scripts/qemu_runtime.sh"
require_grep "9ecd1852625a19430006fa1b2b94a7ae49e459e7044eae17db602572a987b95c" "$REPO_ROOT/scripts/qemu_runtime.sh"
require_grep "cdf057a71b07e3b52b19cbe210bdefa59250d01a9810b960f7fe1f98eed95a27" "$REPO_ROOT/scripts/qemu_runtime.sh"
require_grep "e7e2881a477678dd7daad694dc98e61109d87b74a91443d0cf082802a8991d6a" "$REPO_ROOT/scripts/qemu_runtime.sh"
require_grep "QEMU_BIOS_DIR must be explicitly set" "$REPO_ROOT/scripts/qemu_runtime.sh"
require_grep "qemu_runtime_record" "$REPO_ROOT/scripts/qemu_runtime.sh"
require_grep "qemu_runtime_validate" "$REPO_ROOT/scripts/qemu_runtime.sh"
require_grep "runtime_executed.*true" "$REPO_ROOT/scripts/qemu_runtime.sh"
require_grep "test-qemu-preflight:" "$REPO_ROOT/Makefile"
require_grep "QEMU_PREFLIGHT_UNSET_BLOCKED" "$REPO_ROOT/scripts/qemu_runtime_preflight_test.sh"
require_grep "QEMU_PREFLIGHT_HASH_BLOCKED" "$REPO_ROOT/scripts/qemu_runtime_preflight_test.sh"
require_grep "QEMU_PREFLIGHT_EARLY_EXIT_BLOCKED" "$REPO_ROOT/scripts/qemu_runtime_preflight_test.sh"
require_grep "QEMU_PREFLIGHT_EMPTY_SERIAL_BLOCKED" "$REPO_ROOT/scripts/qemu_runtime_preflight_test.sh"
require_grep "QEMU_PREFLIGHT_MISSING_MARKER_BLOCKED" "$REPO_ROOT/scripts/qemu_runtime_preflight_test.sh"

runtime_scripts="address_space_test.sh allocator_test.sh boot_test.sh e820_test.sh elf_loader_test.sh exception_test.sh exec_args_test.sh marker_surface_test.sh memory_test.sh paging_test.sh process_fd_test.sh process_lifecycle_test.sh process_syscall_test.sh ramdisk_test.sh red_team_test.sh scheduler_safety_test.sh scheduler_test.sh shell_io_test.sh shell_test.sh syscall_file_test.sh syscall_negative_test.sh syscall_test.sh timer_preemption_test.sh timer_test.sh usermode_test.sh vfs_test.sh vm_test.sh"
for script in $runtime_scripts; do
  require_grep "qemu_runtime[.]sh" "$REPO_ROOT/scripts/$script"
  require_grep "qemu_runtime_preflight" "$REPO_ROOT/scripts/$script"
  require_grep "qemu_runtime_begin" "$REPO_ROOT/scripts/$script"
  require_grep "qemu_runtime_verify" "$REPO_ROOT/scripts/$script"
done

if rg -n "SKIP_QEMU|opt/share/qemu|QEMU_BIOS_DIR:-/|timeout --preserve-status" "$REPO_ROOT/scripts" >/dev/null 2>&1; then
  fail "runtime scripts contain a QEMU bypass, fallback, or status-masking path"
else
  pass "runtime scripts require explicit fail-closed QEMU execution"
fi

require_grep "test-marker-surface:" "$REPO_ROOT/Makefile"
require_grep "test-deep:.*test-marker-surface.*test-exec-args" "$REPO_ROOT/Makefile"
require_grep "MARKER_SURFACE_ENOSYS_OK" "$REPO_ROOT/scripts/marker_surface_test.sh"
require_grep "ENABLE_MARKER_SURFACE_SELFTEST" "$REPO_ROOT/kernel/kernel.c"
require_grep "test_marker_permissions" "$REPO_ROOT/include/process.h"
require_grep "process_consume_test_marker" "$REPO_ROOT/kernel/syscall.c"
require_grep "SYSCALL_E2BIG" "$REPO_ROOT/include/syscall.h"
require_grep "EXEC_MAX_VECTOR_BYTES[[:space:]]*3072" "$REPO_ROOT/include/syscall.h"
require_grep "exec_copy_vector" "$REPO_ROOT/kernel/syscall.c"
require_grep "exec_build_user_stack" "$REPO_ROOT/kernel/syscall.c"
require_grep "EXEC_ARGS_LAYOUT_OK" "$REPO_ROOT/scripts/exec_args_test.sh"
require_grep "EXEC_ARGS_CONTENT_OK" "$REPO_ROOT/scripts/exec_args_test.sh"
if grep -q "SYSCALL_TEST_MARKER_TOKEN" "$REPO_ROOT/include/syscall.h"; then
  fail "retired public marker token remains in the syscall ABI"
else
  pass "public marker token is retired"
fi

if grep -Eq '^test-deep:.*test-red-team' "$REPO_ROOT/Makefile"; then
  fail "test-red-team must stay separate from functional test-deep"
else
  pass "test-red-team is separate from functional deep gates"
fi
if grep -Eq '^test-deep:.*test-blue-team' "$REPO_ROOT/Makefile"; then
  fail "test-blue-team must stay separate from functional test-deep"
else
  pass "test-blue-team is separate from functional deep gates"
fi
...
require_grep 'waitpid-signals-pipes' "harness_profile.yaml"
require_grep 'test-vfs' "11-reference/README.md"
require_grep 'test-red-team' "11-reference/README.md"
require_grep 'test-vm' "11-reference/README.md"
require_grep 'test-vm' "13-agent-routing-and-risk/README.md"
...
require_grep 'make test-static-analysis' "harness_profile.yaml"
require_grep 'make test-red-team-tooling' "harness_profile.yaml"
require_grep 'make test-meta-loop' "harness_profile.yaml"
require_grep 'RT-TOOLING-001' "harness_profile.yaml"
require_grep 'RT-HARNESS-002' "harness_profile.yaml"
require_grep 'test-static-analysis:' "$REPO_ROOT/Makefile"
require_grep 'test-red-team-tooling:' "$REPO_ROOT/Makefile"
require_grep 'test-meta-loop:' "$REPO_ROOT/Makefile"
require_grep 'STATIC_SYNTAX_ALL_FEATURES_OK' "$REPO_ROOT/scripts/static_analysis_test.sh"
require_grep 'STATIC_HEADER_PROVENANCE_OK' "$REPO_ROOT/scripts/static_analysis_test.sh"
require_grep 'RED_TOOLING_HEADER_SHADOW_CONFIRMED' "$REPO_ROOT/scripts/tooling_red_team_test.sh"
require_grep 'RED_ADDRSPACE_ORACLE_HARDENED' "$REPO_ROOT/scripts/tooling_red_team_test.sh"
require_grep 'ffreestanding' "$REPO_ROOT/.vscode/c_cpp_properties.json"
require_grep 'RT-TOOLING-001' "red-team/ATTACK_CATALOG.md"
require_grep 'RT-HARNESS-002' "red-team/ATTACK_CATALOG.md"
require_grep 'RT-TOOLING-001' "blue-team/CURRENT_FINDINGS.md"
require_grep 'RT-HARNESS-002' "blue-team/PATCH_PLAYBOOKS.md"
require_grep 'Observe' "14-meta-loop/README.md"
require_grep 'Feed back' "14-meta-loop/README.md"
...
if command -v rg >/dev/null 2>&1; then
  ...
  rg -n 's[e]cure[[:space:]]+OS|security[[:space:]]+compl[e]te|production-grade[[:space:]]+security|fully[[:space:]]+isolated' . --glob '!**/06-validation/**' --glob '!**/11-reference/**' && fail "broad security claim found" || pass "no broad security claim"
